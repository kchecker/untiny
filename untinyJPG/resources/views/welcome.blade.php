...
<div class="w-full bg-gray-800 rounded-2xl container-shadow overflow-hidden mb-5">
  <div>
    ...
    <form method="POST" action="/upscale_image" enctype="multipart/form-data" class="min-h-[380px]" id="myForm" novalidate>
      @csrf
      <!-- Upload Section -->
      <div class="bg-gray-900 p-6 justify-center">
        ...
        <div id="dropArea"
          ...
          role="button" tabindex="0" aria-label="Upload image by drag and drop or browse files">
          ...
          <button type="button" id="browseBtn"
            class="upload-button bg-sky-600 hover:bg-sky-500 text-white px-5 py-2 rounded-lg text-sm shadow-md transition">
            Browse Files
          </button>
        ...
        <!-- Preview -->
        <div id="imagePreviewContainer" class="hidden mt-6 relative flex flex-col items-center" aria-live="polite">
          <div
            class="relative w-full h-64 flex justify-center items-center bg-gray-800 rounded-xl border-2 border-dashed border-sky-500">
            <img id="previewImage" class="max-h-full max-w-full object-contain rounded-lg" alt="Selected image preview" />
            <button id="removeImageBtn" type="button"
              class="absolute top-3 right-3 hover:scale-110 transition"
              title="Remove image" aria-label="Remove selected image">
